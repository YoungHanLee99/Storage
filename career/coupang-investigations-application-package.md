...
**② 커버레터에 내부조사 실적이 반영되어 있습니다.**

d'strict CLO로서의 **그룹 내부조사 수행·종결**과 **직장 내 괴롭힘 신고 사건 다수 조사**가 §1·§2 커버레터, 국문 경력기술서, 영문 CV에 모두 들어가 있습니다. 필수요건 ③④가 이로써 충족됩니다. 건수·규모 등 숫자를 더하면 더 강해지지만(§4), 현 상태로 제출 가능합니다.
...
## 4. 선택 보강 — 숫자를 더하면 더 강해집니다

현재는 **"다수 조사"** 로 서술되어 있습니다. 사실에 부합하고 그대로 제출 가능하지만, 아래가 들어가면 서류가 한 단계 올라갑니다.

1. **연간/누적 건수** — 내부조사 총 N건, 그중 괴롭힘 등 HR 사건 N건
2. **규모** — 인터뷰 인원, 검토 자료 건수, 관련 금액, 소요 기간
3. **결과** — 징계 / 형사고소 / 제도개선, 사후 분쟁(노동위원회 등) 유무
4. **데이터 기반 사례 1건** — 거래·정산·로그·근태 자료에서 이상 징후를 잡아 결론이 바뀐 건 *(필수요건 ⑤ 직결)*
